...
4) Run the code below and use the typeof() and/or class() function (see it's help) and see how R teats each newly-created vector?
```{r}
num_char <- c(1, 2, 3, "a")
...
tricky <- c(1, 2, 3, "4")
```
You should find that R coerces the data to a lowest common denominator - can you work out the heirachy?     

5) You have been asked to plot some base R graphs of counts data. The investigator has requested that certain symbols be used for each dataset being plotted.    
...
6) Your colleague has supplied you with the following table of data (number of cells per sample volume):   
