...
```{r scatter_plot_1}
plot <- ggplot(data = metabric, mapping = aes(x = FOXA1, y = MLPH, colour = ER_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
  labs(
    title = "mRNA expression in the METABRIC breast cancer data set",
    subtitle = "FOXA1 and MLPH mRNA expression measured using Illumina HT-12 bead arrays",
    x = "log2 FOXA1 expression",
    y = "log2 MLPH expression",
    colour = "ER status"
  )
plot
```
...
```{r scatter_plot_2}
ggplot(data = metabric, mapping = aes(x = FOXA1, y = MLPH, colour = ER_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
...
case we wanted numbers from 6 to 14 going up in steps of 2.
...
lternative themes next week).

```{r scatter_plot_4}
ggplot(data = metabric, mapping = aes(x = FOXA1, y = MLPH, colour = ER_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(4, 14, by = 2), minor_breaks = seq(4.5, 15, 0.5), limits = c(4, 15))
```
...
ggplot(data = metabric, mapping = aes(x = FOXA1, y = MLPH, colour = ER_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(4, 14, by = 2), minor_breaks = NULL, limits = c(4, 15))
```
...
A more common scenario would be to keep the breaks, because we want to display
...
ggplot(data = metabric, mapping = aes(x = FOXA1, y = MLPH, colour = ER_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(4, 14, by = 2), limits = c(4, 15)) +
