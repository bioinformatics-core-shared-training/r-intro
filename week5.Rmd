...
```{r}
2 ** (10.64 - 6.21)  # equivalent to (2 ** 10.64) / (2 ** 6.21)
```
...
other genes of interest, this time using the PAM50 classification to define the
groups.
...
ggplot(data = metabric, mapping = aes(x = GATA3, y = ESR1, colour = ER_status)) +
  geom_point(size = 0.6, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(5, 15, by = 2.5), limits = c(4, 14))
```
