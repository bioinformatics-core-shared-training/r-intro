...
returning a data frame (or tibble) that can then be used in subsequent steps in
a 'piped' workflow including passing into `ggplot()` to create a graph.
...
expression within specified groups, so you can see if you're trying to draw
...
dplyr. The name column identifies each of the band members and is used for
matching rows from the two tables.
...
Unlike `left_join()`, `right_join()` is used very infrequently.
...
want to join, columns which are not the variable being used to identify which
observations correspond to each other across the tables. These variables may
...
band_members5 <- tibble(
  ...
  band = c("Beetles", "Beetles", "Beetles", "Beetles", "Stones")
)
...
Care should be taken to rename or remove the duplicated columns that aren't used
for joining.
...
into a single data frame using an `inner_join()`.
...
The labels are another component of the plot object that we've constructed with
aesthetic mappings and layers (geoms). The plot object is a list and contains
various elements including those mappings and layers and one element named
`labels`.
...
The x and y variables are continuous hence ggplot2 adds a continuous scale for
each. Colour is a discrete variable in this case so ggplot2 adds a discrete
scale for colour.
...
Look at the help page for `scale_y_continuous` so see what we can change about
the y axis scale.
...
`seq()` is a useful function for generating regular sequences of number. In this
case we wanted numbers from 5 to 15 going up in steps of 2.5.
...
ggplot(data = metabric, mapping = aes(x = GATA3, y = ESR1, colour = ER_status)) +
  geom_point(size = 0.6, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(5, 12.5, by = 2.5), minor_breaks = seq(4, 14, 0.5), limits = c(4, 14))
```
...
ggplot(data = metabric, mapping = aes(x = GATA3, y = ESR1, colour = ER_status)) +
  geom_point(size = 0.6, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_y_continuous(breaks = seq(4, 14, by = 2), minor_breaks = NULL, limits = c(4, 14))
```
...
the labels, but remove the grid lines. Somewhat confusingly the position of grid
lines are controlled by a scale but preventing these from being displayed
requires changing the theme. We'll look at how the theme controls how non-data
components of a plot can be customized next week but, for now, here's an example
of turning off the display of all grid lines for major and minor breaks for both
axes and also the tick marks.
...
more space, e.g. 10% (0.1), as follows.
...
We can move draw the axis on the other side of the plot –- not sure why you’d
want to do this but with ggplot2 just about anything is possible.
...
Setting colours manually when we only have a two or three categories but when we
have a larger number it would be handy to be able to choose from a selection of
carefully-constructed colour palettes. Helpfully, ggplot2 provides access to the
[ColorBrewer](http://colorbrewer2.org) palettes through the functions
`scale_colour_brewer()` and `scale_fill_brewer()`.
...
ggplot(data = metabric, mapping = aes(x = GATA3, y = ESR1, colour = ER_status)) +
  geom_point(size = 0.6, alpha = 0.5) +
  geom_smooth(method = "lm") +
  scale_colour_manual(values = c("green", "orange"), labels = c("ER-negative", "ER-positive")) +
  labs(colour = NULL)  # remove legend title for colour now that the labels are self-explanatory
```

What we have done here is to set our own set of mappings from levels in the data
to aesthetic values.

For continuous variables we need to be able to set colours used in the colour
gradient. To demonstrate this we'll correct the Nottingham prognostic index
...
metabric %>%
  ...
  ggplot(mapping = aes(x = GATA3, y = ESR1, colour = Nottingham_prognostic_index)) +
  geom_point(size = 0.5) +
  scale_colour_gradient2(low = "blue", mid = "grey80", high = "red", midpoint = 4.5)
```
...
* Customizing plots created using ggplot2 by changing labels, scales and colours
