...
You can also `select()` to completely reorder the columns so they're in the
...
formula for the Nottingham prognostic index uses the tumour size in centimetres.
We can convert to `Tumour_size` to centimetres by dividing by 10.
...
We'll recalculate the Nottingham prognostic index using the values in our
...
higher degree of precision than is strictly needed.
...
data set from centimetres to millimetres. We'd need to use what is known as an
anonymous function for this. Anonymous functions are also known as lambda
expressions. Usually, an anonymous function is a concise formula used when there
is no equivalent function available in R.

There is no 'multiply by 10' function so we use an anonymous function instead
(anonymous because it has no name, it's an _in situ_ function only used in our
`mutate_` function call.
...
The **`~`** denotes that we're using an anonymous function and the `.` is a
placeholder for the column being operated on. In this case, we're multiplying
each of the columns between `Sepal.Length` and `Petal.Width` inclusive by 10.
...
variables (columns) in a table. Here we will summarize values for the entire
...
grouping in week 6 where we'll use `group_by()` in combination with both
`summarize()` and `mutate()`.

Any function that calculates a single scalar value from a vector can be used
with `summarize()`. For example, the `mean()` function calculates the arithmetic
...
operation. These work in much the same way at their `mutate` cousins.
...
**`summarise_all()`** summarizes values in all columns.
...
You have to be careful with `summarise_all()` that all columns can be summarized
...
```{r}
summarise_at(metabric, vars(FOXA1, MLPH), list(mean, sd))
```

Pretty neat but I'm not sure about those column headings -- fortunately we have
some control over these.

```{r}
summarise_at(metabric, vars(FOXA1, MLPH), list(average = mean, stdev = sd))
```

## Anonymous functions

The `mutate_` functions and `summarize_` functions work in a very similar
manner, very much in line with the coherent and consistent framework provided
provided by `dplyr` and the entire tidyverse. For example, we could use an
anonymous function in a `summarize_at()` function applied to multiple variables.
In the assignment from last week, we asked you to compute the correlation of the
expression for FOXA1 against all other genes to see which was most strongly
correlated. Here is how we could do this in a single `summarize_at()` statement
using an anonymous function.

```{r}
summarize_at(metabric, vars(ESR1:MLPH, -FOXA1), ~ cor(., FOXA1))
```
...
An alternative is to used faceting with **`facet_wrap()`**.
...
which points which belong to each group. A case in point is for the 3-gene
