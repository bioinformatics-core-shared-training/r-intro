...
hence the first argument of almost every function that works on data frames has
...
anonymous function in a `summarise_at()` function applied to multiple variables.
...
```{r scatter_plot_11}
metabric %>%
  mutate(HER2_status = str_c("HER2 ", HER2_status)) %>%
  mutate(Neoplasm_histologic_grade = str_c("Grade ", Neoplasm_histologic_grade)) %>%
  ggplot(mapping = aes(x = GATA3, y = ESR1, colour = PR_status)) +
  geom_point(size = 0.5, alpha = 0.5) +
  facet_grid(Neoplasm_histologic_grade ~ HER2_status)
```

We also changed the categories for the variables used for faceting so their
meaning is clearer by prefixing the HER2 status values with "HER2 " and
prefixing the histologic grade with "Grade " using the `str_c()` function.
Notice also how we did these on-the-fly modifications as part of a piped
workflow and passed the resulting data frame into the `ggplot()` function,
which no longer required us to specify it's first argument (`data`).
