...
* **NPI** is the Nottingham prognostic index
...
It appears that the conversion of tumour size into centimetres was not carried
out in the original NPI calculation. This could explain why the plots we created
in weeks 1 and 2 that featured the Nottingham prognostic index looked so odd.
We'll see if they look any better with our recalculated values.
...
functions, are really very powerful but with that comes some complexity. For
example, we may come across situations where we'd like to apply the same
operation to multiple columns but where there is no available function in R
to do what we want.

Let's say we want to conver the petal and sepal measurements in the `iris`
data set from centimetres to millimetres. We'd need to use what is known as a
lambda expression for this.
...
The **`~`** denotes that we're using a lambda expression and the `.` is a
...
# Computing summary values using `summarize()`

---

# Writing tabular data to files
