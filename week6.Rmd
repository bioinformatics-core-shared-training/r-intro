...
TODO: explain what we're trying to achieve, i.e. the end goal

We're going to focus on just a few proteins, those whose levels are markedly
reduced 24 hours after treatment with tamoxifen compared with the vehicle.
...
> `str_replace("Oscar is the best dog in the world", "dog", "cat")`
>
> ` [1] "Oscar is the best cat in the world"`
...
vector of character values (or strings), so let's try this on some of our sample
names (the first 10).
...
concatenate the two using another `stringr` function, `str_c()`. But we'll also
shorten 'vehicle' to 'veh' so we have similar length treatment labels ('veh' and
'tam') in the plot we'll eventually create. For that we'll use `str_sub()`.
