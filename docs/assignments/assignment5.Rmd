...
This week's assignment will give you a chance to summarize values for groups of
observations in the METABRIC data set. We'll also introduce a second data set
from the METABRIC study that contains mutations detected from targeted
sequencing of a panel of 173 genes for 2433 breast cancer samples.
...
clinical data that you should by now be quite familiar with.
...
The second contains the mutations that were detected in each patient tumour
sample.
...
sequenced, so we've also read the case list of samples that were sequenced and
add a column indicating which were sequenced in the clinical data table.

```{r}
cases_sequenced <- read_tsv("../data/cases_nat_comm_2016.txt", col_names = "Patient_ID")
metabric <- mutate(metabric, Sequenced = Patient_ID %in% cases_sequenced$Patient_ID)
count(metabric, Sequenced)
```

The clinical data, gene expression values and mutation data were downloaded from
[cBioPortal](https://www.cbioportal.org/study/summary?id=brca_metabric).

---

**1. Compare the average survival time and other characteristics between ER-negative and ER-positive patients**
...
Some genes contain multiple mutations within a single tumour sample so we could
be over-counting the numbers of samples containing a mutated gene. Modify your
calculation to prevent this double-counting.
...
the mutations occur randomly across these genes or are they usually found in
specific locations?
...
_Hint: take a look at what data are available in the `mutations` table_
...
Customize your plot with a title and axis labels that look a bit more
professional (and don't contain underscores).

Set the breaks on the y-axis to be at 20% intervals. Change the default
behaviour to add 5% space on each side of the plot for continuous variables so
that the Integrative Cluster labels are closer to the bars.

Choose a colour scheme for the bars (try `scale_fill_viridis_d`).

The final two touches make stylistic changes to the theme using customizations
that we'll cover next week. Remove the grey background by changing to the
minimal theme and remove the grid lines on the x-axis that appear as vertical
lines behind each of the bars by adding the following lines to your ggplot.

```
# don't add code here - these are the two lines of code to add to your
# plot to remove the grey background and grid lines
plot +
  theme_minimal() +
...
Check how well the plot resembles that in figure 5a of the METABRIC mutation
paper (note that only 9 of the Integrative Clusters are shown).
...
Download supplementary table S3 and create a scatter plot comparing the TSG
scores the `allSamples_TSGScore` column with those you've calculated.
