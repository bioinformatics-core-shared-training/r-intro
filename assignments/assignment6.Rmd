...
Remove all rows with missing values for the number of tuberculosis cases.
...
Kingdom with separate lines for each age group. The year should be on the
x axis and the number of cases on the y axis.
...
Create a combined data set containing the ER status, expression value and copy
number state for each patient and gene pairing. The data frame should contain
the columns Sample, ER_status, Gene, Expression and Copy_number_state.
...
There will be chocolate-based prizes awarded to any assignment handed in with a
plot that is prettier than mine -- Matt (bear in mind that I'm also the judge
and jury).
