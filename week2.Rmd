...
And talking of lengths, one function you're likely to use is the `length()`
function, which unsurprisingly returns the numbers of elements in a vector.
...
pages for those functions make use of these. Two such data frames that are often
used in example code snippets are **`iris`** and **`mtcars`**. To bring them to
the foreground, you can just start using them by name.
...
And just for completeness, here's an example of a help page that uses `iris` in
one of its examples (look at the very last line where the mysterious `iris`
appears, potentially causing a lot of confusion to the uninitiated).
...
This was of accessing the data frame makes more sense when we look at how
...
Another handy hint: you can look at one of a data frames listed in the
Environment tab pane by double-clicking on it.
...
* viewing by double-clicking in the Environment pane
