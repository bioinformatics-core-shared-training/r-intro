...
```{block type = "rmdblock"}
Note: use **`()`** for functions and **`[]`** for subsetting
...
However, in a real Rscript, we might not want to hard-wire the threshold of 0.5
but instead to let the user specify this each time the script is run, e.g. by
