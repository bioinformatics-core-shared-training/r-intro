...
to iterate over and perform the calculation for each value in turn, but not in
R.

Another way of writing the above statement to get the same result would be to
multiple our numbers by themselves.
...
`1`. It uses the second vector 5 times in what is referred to as **_vector
recycling_**.
...
`my_first_list` has 5 elements and when printed out like this looks quite
