...
often cryptic equivalent using traditional 'base R'. This old-timer only really
...
You may be thinking that that last example extracting a subset using logical
values seems very abstract and why on earth it could possibly be useful. But
actually, it is probably the most frequently used way of selecting values of
...
This reveals that `iris` is a rather famous old data set from measurements taken
...
row numbers are all displayed in quotation marks? They are in fact characters.
...
because it can't find columns for the Ferrari or the Maserati (give it go and
see for yourself).
...
selecting the first five rows and the first five columns.

```{r}
mtcars[1:5, 1:5] # equivalent to mtcars[c("Mazda RX4", "Mazda RX4 Wag", "Datsun 710", "Hornet 4 Drive", "Hornet Sportabout"), c("mpg", "cyl", "disp", "hp", "drat")]
```
...
One last aside before moving on the more user-friendly tidyverse version of the
data frame (the tibble). Earlier we truncated the data frame when printing it
...
right-hand side in RStudio) -- double-clicking on a data frame in this pane will
also open your object in the viewer.
...
able to use the functions and data sets it provides. The tibble data structure
is part of the **`tibble`** package so to make this available you would run the
following command:
...
The package also contains various useful functions for converting other data
structures, e.g. data frames, to tibbles and for working with tibble objects.
...
There is also a warning message about some conflicts. These are functions
loaded as part of the `dplyr` package that have the same name as functions that
we were previously able to access from the `stats` package that is automatically
loaded when R starts. Those functions from the `stats` package are now masked;
to use these instead of their `dplyr` namesakes you need to specify them by
adding the package name and `::` as shown in the warning message.
...
working directory and which you can get by running the **`getwd()`** function.

```{r eval = FALSE}
getwd()
```

It is quite likely your working directory is your home folder.
...
```{r eval = FALSE}
setwd("/Users/matt/projects/20200119_JB_amplicon_sequencing")
```

If you're running an R script from a terminal window (by typing something like
`Rscript my_script.R` at the command prompt) you will most likely want to read
files from locations relative to the directory in which you run the script.
...
table you can see 2 missing values for mutation counts within the first 10 rows.
...
missing values in a vector. Can you see why this works?
