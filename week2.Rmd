...
out, usually replaced by another line that does something similar.
...
It is a collection of values or things of the same type in a given order. We
created some last week using `c()` and the `:` operator.
...
From the help page you can see that `c` stands for 'combine'.

One of the most useful things about the help pages for functions are the
examples given -- you'll need to scroll down to the bottom of the page. These
can be really helpful in demonstrating how a function works. You can very
easily cut and paste these examples and run these in the console window as a
way of experimenting with the function.
...
on, we'll introduce another type of data structure that don't have this
...
characters everything else gets 'coerced' to becoming characters also.
