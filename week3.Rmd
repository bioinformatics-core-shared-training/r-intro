...
3-gene classification, or when we created separate box plots in the same plot
...
Tumour stage has only 5 discrete states but ggplot2 doesn't know this and has
used a colour scale to show these as if they were continuous. We need to tell R
that these are categorical.

R has a special type for categorical variables called a **factor**. Let's
convert our tumour stage variable to a factor using `as.factor()`.
